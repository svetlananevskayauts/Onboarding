...
if (
  ...
  console.error(
    "Missing required .env values: AIRTABLE_TOKEN, AIRTABLE_BASE, AIRTABLE_STARTUPS_TABLEID, AIRTABLE_MEMBERS_TABLEID",
  );
  process.exit(1);
}
    ...
    cp.on("exit", (code) => {
      if (code === 0) {
        ...
        log(
          "warn",
          "sky.refresh.error",
          { code, stderr: err.trim().slice(0, 200) },
          req,
        );
        resolve(false);
      }
...
async function ensureSkyTokenFresh(req) {
  ...
  let exp =
    parseIso(expIso) || decodeJwtExp(process.env.SKY_ACCESS_TOKEN || "");
...
app.post("/discount-check", async (req, res) => {
  try {
    ...
    let memberRec = null;
    if (memberRecordId)
      memberRec = await getRecord(AIRTABLE_MEMBERS_TABLEID, memberRecordId);
    ...
    const email =
      ...
      fieldStr(
        memberRec,
        process.env.AIRTABLE_MEMBERS_PRIMARY_EMAIL_FIELD ||
          AIRTABLE_MEMBERS_PRIMARY_EMAIL_FIELD ||
          "UTS Email",
      ) ||
      "";
    const name =
      ...
      fieldStr(
        memberRec,
        process.env.AIRTABLE_MEMBERS_NAME_FIELD ||
          AIRTABLE_MEMBERS_NAME_FIELD ||
          "Name",
      ) ||
      "";
    const dob =
    ...
    log(
      ...
      { memberRecordId: memberRecordId || null, has_search_id: !!search_id },
      req,
    );
    let result = await validator.validateDiscount(
    ...
    if (memberRecordId && updateAirtable) {
      try {
        ...
        log(
          "info",
          "airtable.validation.persist",
          { memberRecordId, duration_ms: Date.now() - a0 },
          req,
        );
      } catch (e) {
        ...
        log(
          "error",
          "airtable.validation.error",
          { memberRecordId, message: e.message },
          req,
        );
      }
    }
    ...
    try {
      const steps =
        result && result.trace && result.trace.steps
          ? result.trace.steps
          : null;
      if (steps && steps.search)
        log(
          "debug",
          "sky.search.finish",
          { status: steps.search.status, count: steps.search.count },
          req,
        );
      if (steps && steps.codes)
        log(
          "debug",
          "sky.codes.finish",
          { status: steps.codes.status, count: steps.codes.count },
          req,
        );
    } catch (_) {}
    log(
      "info",
      "discount_check.finish",
      { status: result.status, valid: !!result.valid, duration_ms: duration },
      req,
    );
    return res.json({
...
app.post("/validate-and-generate", async (req, res) => {
  try {
    ...
    if (startupRecordId) {
      try {
        const team =
          await listMembersWithOnboardingForStartupRobust(startupRecordId);
        const wanted = new Set((team || []).map((r) => r.id));
        const have = new Set(
          (list || []).map((v) => v && v.memberRecordId).filter(Boolean),
        );
        ...
        if (!Array.isArray(validations) || validations.length === 0) {
          ...
          log(
            "info",
            "validate_and_generate.autofill_validations",
            { team_members: list.length },
            req,
          );
          ...
          log(
            "info",
            "validate_and_generate.append_missing_validations",
        ...
        log(
          "warn",
          "validate_and_generate.ensure_all_validated_error",
          { message: String(e.message || e) },
          req,
        );
      }
    ...
    log(
      "info",
      "validate_and_generate.start",
      {
        startupRecordId: startupRecordId || null,
        memberRecordId: memberRecordId || null,
        validations: list.length,
      },
      req,
    );
    for (const v of list) {
      ...
      let mRec = null;
      if (mId && !search_id) {
        mRec = await getRecord(AIRTABLE_MEMBERS_TABLEID, mId).catch(() => null);
      }
      const sid =
        search_id ||
        fieldStr(
      ...
      const v0 = Date.now();
      let result = await validator.validateDiscount(
        { search_id: sid, expected_bucket: exp, email: eml, name: nm, dob: db },
        { debug },
      );
      if (
        (result &&
      ...
      log(
        ...
        "discount_check.finish",
        {
          memberRecordId: mId || null,
          status: result.status,
          valid: !!result.valid,
          duration_ms: Date.now() - v0,
        },
        req,
      );
      let updateRes = null;
      if (updateAirtable && mId) {
        try {
          ...
          log(
            "info",
            "airtable.validation.persist",
            { memberRecordId: mId, duration_ms: Date.now() - a0 },
            req,
          );
        } catch (e) {
          ...
          log(
            "error",
            "airtable.validation.error",
            { memberRecordId: mId, message: e.message },
            req,
          );
        }
      }
      results.push({
        memberRecordId: mId || null,
        input: { sid, exp, eml, nm, db },
        result,
        airtableUpdate: updateRes,
      });
    }
    ...
    log(
      "info",
      "generator.pdf.ready",
      { duration_ms: Date.now() - g0, bytes: pdfBuffer.length },
      req,
    );
    ...
    if (saveLocal) {
      try {
        ...
        const safeName = String(suggested).replace(
          /[<>:"/\\|?*\u0000-\u001F]/g,
          "_",
        );
        ...
        log(
          "warn",
          "generator.pdf.save_local_failed",
          { message: String(e.message || e) },
          req,
        );
      }
    ...
    try {
      ...
      if (!targetStartupId && memberRecordId) {
        try {
          const memberRec = await getRecord(
            AIRTABLE_MEMBERS_TABLEID,
            memberRecordId,
          );
          const linkField =
            typeof AIRTABLE_MEMBER_TO_STARTUP_LINK_FIELD !== "undefined"
              ? AIRTABLE_MEMBER_TO_STARTUP_LINK_FIELD
              : "Startup";
          const linked =
            memberRec?.fields?.[linkField] ||
            memberRec?.fields?.["Startup*"] ||
            [];
          if (Array.isArray(linked) && linked.length)
            targetStartupId = String(linked[0]);
          else if (typeof linked === "string" && linked.startsWith("rec"))
            targetStartupId = linked;
        } catch (_) {
          /* ignore */
        }
      }
      if (targetStartupId) {
        ...
        const existing = Array.isArray(rec?.fields?.[STARTUPS_AGREEMENT_FIELD])
          ? rec.fields[STARTUPS_AGREEMENT_FIELD]
          : [];
        ...
        const updated = await updateRecord(
          AIRTABLE_STARTUPS_TABLEID,
          targetStartupId,
          { [STARTUPS_AGREEMENT_FIELD]: [...existing, attachment] },
        );
        const count = Array.isArray(updated?.fields?.[STARTUPS_AGREEMENT_FIELD])
          ? updated.fields[STARTUPS_AGREEMENT_FIELD].length
          : 0;
        attachmentResult = {
          ok: true,
          tableId: AIRTABLE_STARTUPS_TABLEID,
          recordId: targetStartupId,
          field: STARTUPS_AGREEMENT_FIELD,
          count,
        };
        log(
          "info",
          "airtable.attachment.persist",
          {
            tableId: AIRTABLE_STARTUPS_TABLEID,
            recordId: targetStartupId,
            field: STARTUPS_AGREEMENT_FIELD,
    ...
    return res.json({
      ...
      airtableAttachment: attachmentResult,
      savedLocal,
    });
  } catch (e) {
    log(
      "error",
      "validate_and_generate.error",
      { message: String(e.message || e) },
      req,
    );
    res.status(500).json({ error: String(e.message || e) });
...
app.post("/pdf-url", async (req, res) => {
  try {
    ...
    log(
      "info",
      "pdf_url.issued",
      { filename: suggested, ttl: ttl, duration_ms: Date.now() - t0 },
      req,
    );
    res.json({
      url: `${baseUrl(req)}/download/${token}`,
...
app.post("/pdf", async (req, res) => {
  try {
    ...
    log(
      "info",
      "pdf.inline.sent",
      { filename: suggested, duration_ms: Date.now() - t0 },
      req,
    );
  } catch (e) {
...
async function buildPayload({ startupRecordId, memberRecordId }) {
  ...
  const [startupRec, memberRec] = await Promise.all([
    startupRecordId
      ? getRecord(AIRTABLE_STARTUPS_TABLEID, startupRecordId)
      : null,
  ...
  if (!startup && memberRec) {
    const linkedStartupId = readFirstLinkedId(
      memberRec,
      AIRTABLE_MEMBER_TO_STARTUP_LINK_FIELD,
    );
    if (linkedStartupId) {
      startup = await getRecord(
        AIRTABLE_STARTUPS_TABLEID,
        linkedStartupId,
      ).catch(() => null);
    }
  ...
  const teamMembers = startup
    ...
    : memberRec
      ? [memberRec]
      : []; // fallback: just the provided member, if any
  ...
  function isRepresentative(val) {
    ...
    const s = String(
      typeof val === "object" && val?.name != null ? val.name : val,
    )
      .trim()
      .toLowerCase();
    return s === "1" || s === "yes";
  ...
  let debtor_name = "";
  const repFromMember =
    memberRec && isRepresentative(memberRec?.fields?.["Representative"]);
  if (repFromMember) debtor_name = nameOf(memberRec);
  else {
    const rep = (teamMembers || []).find((r) =>
      isRepresentative(r?.fields?.["Representative"]),
    );
  ...
  const legal_name =
    fieldStr(
      startup,
      process.env.STARTUPS_LEGAL_NAME_1 || "Registered Business Name",
    ) ||
    fieldStr(
      startup,
      process.env.STARTUPS_LEGAL_NAME_2 || "Registred Business Name",
    ) ||
    "";
  ...
  try {
    ...
    if (sum > 0) {
      calculated_monthly_fee =
        "AUD " + formatMoneyAUD(sum) + " per month plus GST";
    } else {
      const hasFullOrCasual = (teamMembers || []).some((r) => {
        const t = normaliseType(
          fieldStr(
            r,
            process.env.AIRTABLE_MEMBERS_MEMBERSHIP_TYPE_FIELD ||
              "Membership Type",
          ),
        );
  ...
  const team = teamMembers.map(toNameObject).filter(Boolean);
...
async function getRecord(tableId, recordId) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Airtable GET ${tableId}/${recordId} Ã¢â€ â€™ ${res.status} ${res.statusText}: ${t}`,
    );
  }
  return res.json();
}
...
async function listMembersWithOnboardingForStartup(startupRecordId) {
  ...
  const out = [];
  let url = new URL(
    `https://api.airtable.com/v0/${encodeURIComponent(AIRTABLE_BASE)}/${encodeURIComponent(AIRTABLE_MEMBERS_TABLEID)}`,
  );
  ...
  do {
    ...
    if (!res.ok) {
      const t = await res.text().catch(() => "");
      throw new Error(
        `Airtable LIST members Ã¢â€ â€™ ${res.status} ${res.statusText}: ${t}`,
      );
    }
...
async function listMembersWithOnboardingForStartupRobust(startupRecordId) {
  ...
  const filter =
    !DEV_MODE && AIRTABLE_MEMBERS_ONBOARDING_FIELD
  ...
  do {
    ...
    if (!res.ok) {
      const t = await res.text().catch(() => "");
      throw new Error(
        `Airtable LIST members → ${res.status} ${res.statusText}: ${t}`,
      );
    }
    const json = await res.json();
  ...
  const linked = pages.filter((r) => {
    ...
    return links.some((l) =>
      typeof l === "string" ? l === startupRecordId : l?.id === startupRecordId,
    );
  });
...
async function updateRecord(tableId, recordId, fields) {
  ...
  if (!res.ok) {
    const t = await res.text().catch(() => "");
    throw new Error(
      `Airtable PATCH ${tableId}/${recordId} Ã¯Â¿Â½Ã¯Â¿Â½' ${res.status} ${res.statusText}: ${t}`,
    );
  }
  return res.json();
}
...
async function updateMemberValidation(memberRecordId, result, expected, req) {
  ...
  // Minimal storage: boolean flag + optional status + timestamp. No Blackbaud derived data persisted.
  const selection = result.valid
    ? "Valid"
    : result.qualifies_other
      ? "Qualifies for Other"
      : "Invalid";
  ...
  const derived = Array.isArray(result.derived_buckets)
    ? result.derived_buckets
    : [];
  const isAlumni = derived.some((b) => /^UTS Alumni/i.test(String(b)));
  if (isAlumni && result.alumni_expires_at)
    fields[EXPIRES] = String(result.alumni_expires_at).slice(0, 10);
  if (expected) {
    const EXPECTED =
      typeof AIRTABLE_MEMBERS_EXPECTED_DISCOUNT_FIELD !== "undefined"
        ? AIRTABLE_MEMBERS_EXPECTED_DISCOUNT_FIELD
        : "Discount Category";
    fields[EXPECTED] = expected; // keep user selection as ground truth in Airtable
  }
  log(
    "debug",
    "airtable.patch.start",
    {
      tableId: AIRTABLE_MEMBERS_TABLEID,
  ...
  log(
    "debug",
    "airtable.patch.finish",
    { tableId: AIRTABLE_MEMBERS_TABLEID, recordId: memberRecordId },
    req,
  );
  return res;
}

/* Field readers & transformations */
...
function toNameObject(memberRec) {
  ...
  const raw =
    memberRec.fields[AIRTABLE_MEMBERS_NAME_FIELD] ||
  ...
  const first =
    fieldStr(memberRec, "First Name") ||
    fieldStr(memberRec, "First name") ||
    "";
  const last =
    fieldStr(memberRec, "Last Name") || fieldStr(memberRec, "Last name") || "";
  if (!first && !last) return null;
  return { first_name: first, last_name: last };
}

/* Placeholders for calc fields */

function computeMembershipCountsPlaceholder(_memberRecords) {
  const MEMBERSHIP_TYPE_FIELD =
    process.env.AIRTABLE_MEMBERS_MEMBERSHIP_TYPE_FIELD || "Membership Type";
  const DISCOUNT_CATEGORY_FIELD =
    process.env.AIRTABLE_MEMBERS_EXPECTED_DISCOUNT_FIELD || "Discount Category";
  const VALIDATED_SELECT_FIELD =
    process.env.AIRTABLE_MEMBERS_VALIDATED_SELECT_FIELD || "Discount Validated";
  ...
  function isWithin12m(discount) {
    if (!discount) return false;
    return /within the last 12 months|within.*12 months/i.test(
      String(discount),
    );
  }
...
const PRICING_TYPE_FIELD =
  process.env.PRICING_MEMBERSHIP_TYPE_FIELD || "Membership Type";
const PRICING_BASE_FIELD = process.env.PRICING_BASE_RATE_FIELD || "Base Rate";
const COL_CURRENT_STUDENT =
  process.env.PRICING_COL_CURRENT_UTS_STUDENT || "Current UTS Student";
const COL_ALUMNI_WITHIN =
  process.env.PRICING_COL_UTS_ALUMNI_WITHIN_12M || "UTS Alumni < 12m";
const COL_ALUMNI_OVER =
  process.env.PRICING_COL_UTS_ALUMNI_OVER_12M || "UTS Alumni > 12m";
const COL_CURRENT_STAFF =
  process.env.PRICING_COL_CURRENT_UTS_STAFF || "Current Staff";
const COL_FORMER_WITHIN =
  process.env.PRICING_COL_FORMER_STAFF_WITHIN_12M || "Former Staff < 12m";
const COL_FORMER_OVER =
  process.env.PRICING_COL_FORMER_STAFF_OVER_12M || "Former Staff > 12m";

async function loadPricingMatrix() {
  const out = {};
  if (!process.env.AIRTABLE_PRICING_TABLEID) return out;
  let url = new URL(
    `https://api.airtable.com/v0/${encodeURIComponent(AIRTABLE_BASE)}/${encodeURIComponent(process.env.AIRTABLE_PRICING_TABLEID)}`,
  );
  url.searchParams.set("pageSize", "100");
  let offset;
  do {
...
function rateForMember(rec, matrix) {
  ...
  const discountCategory = fieldStr(
    rec,
    process.env.AIRTABLE_MEMBERS_EXPECTED_DISCOUNT_FIELD || "Discount Category",
  );
  const col = discountColumnFor(discountCategory);
  if (
    validationIsValid(rec) &&
    col &&
    row.discounts &&
    row.discounts[col] != null
  ) {
